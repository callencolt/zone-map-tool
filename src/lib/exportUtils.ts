...
export const exportBatchToPDF = async (controllers: ControllerData[], sectionName: string) => {
  const pdf = new jsPDF({
    orientation: 'portrait',
    unit: 'mm',
    format: 'a4',
  ...
  controllers.forEach((controller, index) => {
    ...
    const colWidth = (pageWidth - 30) / 3;
    infoFields.forEach((field, fieldIndex) => {
    ...
    // Reset text color for next controller
    pdf.setTextColor(0, 0, 0);
  });
